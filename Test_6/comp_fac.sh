...
for filename in $(find sim_water | grep fort.7); do
    ...
    echo -e "#\t$folder\t">${folder}/TEMP_DATA.dat       
    echo -e "#\t$folder\t">${folder}/PRESSAVG_PLOT_DATA.dat       
    echo -e "#\t$folder\t">${folder}/KINPRESSAVG_PLOT_DATA.dat
    echo -e "#\t$folder\t">${folder}/KINPRESS_PLOT_DATA.dat
    ...
    cat ${folder}/fort.7 |  grep "    temp"      | awk  ' {print $1 "\t" $2}' >> ${folder}/TEMP_DATA.dat       
    cat ${folder}/fort.7 |  grep "    press_avg"  | awk  ' {print $1 "\t" $2}' >> ${folder}/PRESSAVG_PLOT_DATA.dat   
    cat ${folder}/fort.7 |  grep "    KIN_press "  | awk  ' {print $1 "\t" $2}' >> ${folder}/KINPRESS_PLOT_DATA.dat   
    cat ${folder}/fort.7 |  grep "    KIN_press_avg"  | awk  ' {print $1 "\t" $2}' >> ${folder}/KINPRESSAVG_PLOT_DATA.dat   
